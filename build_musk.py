...
# ── Config ────────────────────────────────────────────────────────────────────
VAULT = Path("/Users/hongxing/Documents/Obsidian Vault/马斯克/马斯克")
OUT   = Path("/Users/hongxing/project/webchat/musk-site")
...
.article{max-width:820px;padding:48px 48px 80px;overflow-wrap:break-word;word-break:break-word}
.meta{
  font-size:13px;
  color:var(--text2);
  margin-bottom:16px;
...
@media(max-width:768px){
  ...
  .main{margin-left:0;max-width:100%;overflow-x:hidden}
  ...
  .fav-wrap{flex-wrap:wrap;gap:6px}
  .sister-link{padding:4px 10px;font-size:11px}
  /* Graph page mobile */
  .main.full-bleed{margin-left:0}
  .graph-title{top:50px;left:12px}
  .graph-title h1{font-size:16px}
  .graph-toolbar{top:50px;right:12px}
  .graph-toolbar button{padding:5px 10px;font-size:11px}
  .graph-legend{bottom:10px;left:10px;right:10px;max-width:none;gap:8px;padding:6px 10px;font-size:10px}
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="/index.html" class="logo">马斯克知识图谱</a></div>
...
<div class="fav-wrap">
  ...
  <a href="https://huang.feima.ai/" class="sister-link" target="_blank" rel="noopener">黄仁勋知识图谱 <span class="arrow">→</span></a>
...
<section class="hero-section">
  ...
  <h1 class="hero-title">马斯克<span class="gold">知识图谱</span></h1>
...
<div class="section">
  ...
  <p style="font-size:14px;color:var(--text2);margin:-8px 0 16px;font-family:var(--serif)">这是马斯克知识图谱相对其他知识图谱独有的维度——可操作的工作方法卡片，每张都附带"你能用上吗？"的实操建议。</p>
  <div class="tag-cloud">
'''
...
def build_graph_page(files):
    ...
    # gold for the "工作方法" dimension that is unique to the Musk knowledge base.
    type_colors = {
        "核心思想":  "#7C5E2A",  # warm brown
        "工作方法":  "#B8860B",  # gold (unique to Musk)
        "产品":      "#1A6B7C",  # teal
        "人物":      "#8B2F2F",  # red
        "总体规划":  "#2A6B4F",  # forest green
        "访谈":      "#4A4E8A",  # indigo
        "财报会议":  "#6B4F7C",  # plum
        "索引":      "#6B6560",  # warm gray
    }
...
def main():
    ...
    assets_src = Path(__file__).parent / "assets-musk"
    if assets_src.is_dir():
        shutil.copytree(assets_src, OUT / "assets")
        print(f"Copied assets-musk/ ({len(list(assets_src.iterdir()))} files)")
