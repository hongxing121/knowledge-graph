#!/usr/bin/env python3
"""
Build a static HTML site from the Naval Ravikant knowledge graph Obsidian vault.
Usage: python3 build_naval.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
VAULT = Path("/Users/hongxing/Documents/Obsidian Vault/纳瓦尔/纳瓦尔")
OUT   = Path("/Users/hongxing/project/webchat/naval-site")
...
.type-著作{background:#2A6B4F}
.type-文章{background:#7E6B30}
...
def build_sidebar_html(files, current_stem=""):
    ...
    groups["book"].sort(key=lambda x: x["stem"])
    groups["podcasts"].sort(key=lambda x: x["stem"])
    groups["essays"].sort(key=lambda x: x["stem"])
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="/index.html" class="logo">纳瓦尔知识图谱</a></div>
...
def build_homepage(files, link_map, ref_counts):
    ...
    sources_total = (
        len([f for f in files if f["category"] == "book"])
        + len([f for f in files if f["category"] == "podcasts"])
        + len([f for f in files if f["category"] == "essays"])
    )

    # Person initials and role
    people_data = {
        "纳瓦尔":            ("N", "AngelList 联合创始人 / 天使投资人 / 哲学家"),
        "Babak Nivi":        ("B", "AngelList / Venture Hacks 联合创始人"),
        "Eric Jorgenson":    ("E", "《纳瓦尔宝典》编纂者"),
        "Tim Ferriss":       ("T", "Tim Ferriss Show 主持人，多次长访谈"),
        "Joe Rogan":         ("J", "JRE #1309 主持人，神级 2 小时长谈"),
        "Shane Parrish":     ("S", "Knowledge Project / Farnam Street 创始人"),
        "Kapil Gupta":       ("K", "医生/思想家，与 Naval 的哲学对谈"),
        "查理·芒格":         ("芒", "Naval 常引用的思想家"),
        "理查德·费曼":       ("费", "第一性原理的代表"),
        "Marc Andreessen":   ("M", "a16z 联合创始人，Naval 的朋友"),
    }
    ...
    html = f'''
<div class="fav-wrap">
  <a href="https://bezos.feima.ai/" class="sister-link" target="_blank" rel="noopener">贝佐斯 <span class="arrow">→</span></a>
  <a href="https://musk.feima.ai/" class="sister-link" target="_blank" rel="noopener">马斯克 <span class="arrow">→</span></a>
  <a href="https://huang.feima.ai/" class="sister-link" target="_blank" rel="noopener">黄仁勋 <span class="arrow">→</span></a>
  <button class="fav-btn" id="fav-btn" aria-label="收藏本站">☆<span class="fav-label">收藏</span></button>
  <div class="fav-pop" id="fav-pop"></div>
</div>

<section class="hero-section">
  <div class="hero-eyebrow">Naval Ravikant · Wealth & Happiness</div>
  <h1 class="hero-title">纳瓦尔<span class="gold">知识图谱</span></h1>
  <p class="hero-sub"><b>{sources_total} 份</b>一手素材，<b>{len(concept_files)} 个</b>核心思想<br>
  从《纳瓦尔宝典》到 Joe Rogan / Tim Ferriss 长访谈——追踪硅谷哲学家的财富与幸福之道</p>
</section>
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>把纳瓦尔散落在《纳瓦尔宝典》、Joe Rogan / Tim Ferriss / Shane Parrish 长访谈、"How to Get Rich" 推文风暴、nav.al 博客中的思想，整理成一张可以漫游的知识图谱。从"专属知识 + 杠杆 = 财富"到"幸福是一种可以训练的技能"——覆盖他关于财富创造与内心平静的完整哲学体系。</p>
    <p>独有视角：<strong>财富与幸福的双轨哲学</strong>——大部分人只知道 Naval 的财富金句，但他关于幸福、冥想、欲望与自由的思考同样深刻，甚至他本人认为 Part II（幸福）比 Part I（财富）更重要。</p>
...
def main():
    ...
    assets_src = Path(__file__).parent / "assets-naval"
    if assets_src.is_dir():
        shutil.copytree(assets_src, OUT / "assets")
        print(f"Copied assets-naval/ ({len(list(assets_src.iterdir()))} files)")
